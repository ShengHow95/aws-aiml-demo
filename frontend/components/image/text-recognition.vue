    ...
    <div class="col-sm-12 text-center mt-3">
      <div class="mx-5">
        ...
        <span class="text-subtitle-1 grey--text text--lighten-1">
          allows you to easily detect text content from images or videos. To
          know more, visit
        </span>
        <a
...
export default {
  ...
  methods: {
    ...
    resultHandler() {
      this.results = []
      if (this.response.TextDetections.length === 0) {
        this.errorText = 'No Object is detected'
        this.snackbar = true
      } else {
        for (let j = 0; j < this.response.TextDetections.length; j++) {
          this.results.push({
            Score: this.response.TextDetections[j].Confidence,
            Name: this.response.TextDetections[j].DetectedText,
          })
        }
      }
    },
